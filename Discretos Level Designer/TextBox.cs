...
namespace Discretos_Level_Designer
{
    public class TextBox
    {
        ...
        public void SetScale(float newScale, float newFrontTickness)
        {
            scale = newScale;
            frontThickness = newFrontTickness;
        }
        ...
        public void SetText(string newText)
        {
            ...
            for (int i = 0; i < newText.Length; i++)
            {
                for (int j = 0; j < characters.Length; j++)
                {
                    if (characters[j] == null)
                    {
                        characters[j] = newText.ToCharArray().GetValue(i).ToString();
                        break;
                    }
            ...
            if(newText.Length > maxCharacters)
                text = newText.Substring(0, maxCharacters);
            else
                text = newText;
