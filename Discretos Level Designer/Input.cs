...
namespace Discretos_Level_Designer
{
    class MouseInput
    {
        ...
        public static Vector2 GetPositionInGridWithZoomAndTranslation(Screen screen, Camera camera)
        {

            float X = GetScreenPosition(screen).X / Camera.Zoom + camera.Position.X - 1920 / (2 * Camera.Zoom);
            float Y = GetScreenPosition(screen).Y / Camera.Zoom + camera.Position.Y - 1080 / (2 * Camera.Zoom);
